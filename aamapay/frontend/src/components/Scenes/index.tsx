...
import { ArrowRight, Handshake, Bitcoin, Coins, Link, Send } from "lucide-react";

export function Scene1({ isActive }: { isActive: boolean }) {
  return (
    <div
      className={`absolute inset-0 flex flex-col items-center justify-center transition-opacity duration-700 ${isActive ? "opacity-100" : "opacity-0"}`}
    >
...
export function Scene2({ isActive }: { isActive: boolean }) {
  return (
    <div
      className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ${isActive ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-20"}`}
    >
        ...
        <div className="flex-1">
          <div className="w-64 h-80 rounded-3xl bg-gradient-to-br from-[#B91C1C] to-[#991B1B] flex items-center justify-center shadow-2xl">
            <div className="w-32 h-48 bg-white/20 rounded-2xl backdrop-blur flex flex-col items-center justify-center gap-4 p-6">
              ...
              <p className="text-white text-sm font-medium">100 SOL</p>
            </div>
          </div>
        </div>
        <div className="flex-1">
          ...
          <p className="text-lg text-[#6B7280] max-w-md">
            Connect your wallet and send SOLONA to your family in Nepal. No bank
            account needed for them to receive.
          </p>
        </div>
      </div>
    </div>
...
export function Scene3({ isActive }: { isActive: boolean }) {
  return (
    <div
      className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ${isActive ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
    >
      <div className="text-center max-w-4xl px-8">
        <div className="relative mb-12">
          <div className="flex items-center justify-center gap-8">
            <div className="w-20 h-20 rounded-2xl bg-[#B91C1C]/10 flex items-center justify-center">
              <Bitcoin className="w-10 h-10 text-[#B91C1C]" />
            </div>
        ...
        <p className="text-xl text-[#6B7280]">
          Your funds are locked in a smart contract until the recipient confirms
          receipt
        </p>
      </div>
    </div>
...
export function Scene4({ isActive }: { isActive: boolean }) {
  return (
    <div
      className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ${isActive ? "opacity-100" : "opacity-0"}`}
    >
      <div className="flex items-center gap-16 max-w-6xl px-8">
        <div className="flex-1">
          <h2 className="text-4xl md:text-5xl font-bold text-[#111827] mb-6">
          ...
          <p className="text-lg text-[#6B7280] max-w-md">
            Share a secure claim code with your recipient. They can withdraw
            from any agent nearby.
          </p>
        </div>
    ...
    >
      <div className="flex items-center gap-16 max-w-6xl px-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-lg text-[#6B7280] max-w-md">
            Your family visits any verified AamaPay agent, shows the claim code,
            and receives cash instantly.
          </p>
        </div>
      </div>
    </div>
...
export function Scene6({ isActive }: { isActive: boolean }) {
  return (
    <div
      className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ${isActive ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
    >
      <div className="text-center max-w-4xl px-8">
