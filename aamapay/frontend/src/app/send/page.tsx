...
export default function SendPage() {
  ...
  const [step, setStep] = useState<'connect' | 'form' | 'processing' | 'success'>('form');
        ...
        <div className="max-w-lg mx-auto px-4">
          <div className="bg-white rounded-3xl shadow-lg p-8 text-center animate-fade-in">
            ...
            <div className="bg-[#F5F5F5] rounded-2xl p-6 mb-6">
              ...
              {txData.transaction?.solanaTx && (
                <div className="flex items-center justify-between">
                  <span className="text-[#6B7280]">Transaction</span>
                  <a 
                    href={`https://solscan.io/tx/${txData.transaction.solanaTx}?cluster=devnet`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#B91C1C] font-mono text-sm"
                  >
                    View on Solscan
                  </a>
                </div>
            ...
            {txData.transaction?.claimCode && (
              <div className="bg-[#B91C1C]/5 border-2 border-[#B91C1C]/20 rounded-2xl p-6 mb-6">
                <p className="text-sm text-[#6B7280] mb-3">Claim Code (if using cash pickup)</p>
                <div className="flex items-center justify-center gap-3">
                  <code className="text-2xl font-mono font-bold text-[#B91C1C]">
                    {txData.transaction.claimCode}
                  </code>
                  <button onClick={copyClaimCode} className="p-2 hover:bg-gray-100 rounded-xl">
      ...
      <div className="max-w-lg mx-auto px-4">
        ...
        <div className="bg-white rounded-3xl shadow-lg overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#374151] mb-2">
                  Recipient Phone / Wallet / Email
                </label>
                <input
                  type="text"
                  value={recipient}
                  onChange={(e) => setRecipient(e.target.value)}
                  placeholder="9800000000 or wallet address"
            ...
            <button
              type="submit"
              disabled={!connected || !amount || !recipient || txLoading}
